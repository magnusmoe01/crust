...
const BESTILLING_INTERNAL_RECIPIENTS = ["event@crust.no", "brandon@crust.no"];

function buildCateringEmailHtml(data) {
  return `
    <html><body style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px;color:#1f2937">
      <h2 style="margin:0 0 16px">Ny cateringforespørsel</h2>
      <table style="width:100%;border-collapse:collapse;">
        <tr><td style="padding:8px 0;font-weight:700;width:140px;">Navn</td><td style="padding:8px 0;">${data.name || ""}</td></tr>
        <tr><td style="padding:8px 0;font-weight:700;">Telefon</td><td style="padding:8px 0;">${data.phone || ""}</td></tr>
        <tr><td style="padding:8px 0;font-weight:700;">E-post</td><td style="padding:8px 0;">${data.email || ""}</td></tr>
        <tr><td style="padding:8px 0;font-weight:700;">Adresse</td><td style="padding:8px 0;">${data.address || ""}</td></tr>
        <tr><td style="padding:8px 0;font-weight:700;">Type</td><td style="padding:8px 0;">${data.cateringType || ""}</td></tr>
        <tr><td style="padding:8px 0;font-weight:700;">Dato</td><td style="padding:8px 0;">${data.date || ""}</td></tr>
        ${data.comments ? `<tr><td style="padding:8px 0;font-weight:700;">Kommentar</td><td style="padding:8px 0;">${data.comments}</td></tr>` : ""}
      </table>
      <p style="margin-top:24px;color:#6b7280;font-size:13px">— Crust bestillingssystem</p>
    </body></html>`;
}

function buildCateringConfirmationHtml(data) {
  return `
    <html><body style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px;color:#1f2937">
      <h2 style="margin:0 0 16px">Takk for din forespørsel!</h2>
      <p>Hei ${data.name || ""},</p>
      <p>Vi har mottatt din cateringforespørsel for <strong>${data.date || "ønsket dato"}</strong>.</p>
      <p>Vi tar kontakt med deg snart for å bekrefte detaljer.</p>
      <p style="margin-top:24px;color:#6b7280;font-size:13px">— Crust n' Trust</p>
    </body></html>`;
}

function buildLargeOrderEmailHtml(data) {
  const snap = data.priceSnapshot || {};
  return `
    <html><body style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px;color:#1f2937">
      <h2 style="margin:0 0 16px">Ny storbestilling</h2>
      <table style="width:100%;border-collapse:collapse;">
        <tr><td style="padding:8px 0;font-weight:700;width:140px;">Navn</td><td style="padding:8px 0;">${data.name || ""}</td></tr>
        <tr><td style="padding:8px 0;font-weight:700;">Telefon</td><td style="padding:8px 0;">${data.phone || ""}</td></tr>
        <tr><td style="padding:8px 0;font-weight:700;">E-post</td><td style="padding:8px 0;">${data.email || ""}</td></tr>
        <tr><td style="padding:8px 0;font-weight:700;">Adresse</td><td style="padding:8px 0;">${data.address || ""}</td></tr>
        <tr><td style="padding:8px 0;font-weight:700;">Pizzaer</td><td style="padding:8px 0;">${data.pizzaQuantity || 0} stk</td></tr>
        <tr><td style="padding:8px 0;font-weight:700;">Brus</td><td style="padding:8px 0;">${data.sodaQuantity || 0} stk</td></tr>
        <tr><td style="padding:8px 0;font-weight:700;">Dressinger</td><td style="padding:8px 0;">${data.dressingQuantity || 0} stk</td></tr>
        <tr><td style="padding:8px 0;font-weight:700;">Levering</td><td style="padding:8px 0;">${data.deliveryDate || ""}</td></tr>
        <tr><td style="padding:8px 0;font-weight:700;">Estimert pris</td><td style="padding:8px 0;font-weight:700;color:#fb7f59;">${snap.estimatedTotal ? snap.estimatedTotal.toLocaleString("nb-NO") + " kr" : "—"}</td></tr>
        ${data.deliveryComments ? `<tr><td style="padding:8px 0;font-weight:700;">Levering</td><td style="padding:8px 0;">${data.deliveryComments}</td></tr>` : ""}
        ${data.selectionComments ? `<tr><td style="padding:8px 0;font-weight:700;">Ønsker</td><td style="padding:8px 0;">${data.selectionComments}</td></tr>` : ""}
      </table>
      <p style="margin-top:24px;color:#6b7280;font-size:13px">— Crust bestillingssystem</p>
    </body></html>`;
}

function buildLargeOrderConfirmationHtml(data) {
  const snap = data.priceSnapshot || {};
  return `
    <html><body style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px;color:#1f2937">
      <h2 style="margin:0 0 16px">Takk for din bestilling!</h2>
      <p>Hei ${data.name || ""},</p>
      <p>Vi har mottatt din storbestilling:</p>
      <ul>
        <li>${data.pizzaQuantity || 0} pizzaer</li>
        ${data.sodaQuantity ? `<li>${data.sodaQuantity} brus</li>` : ""}
        ${data.dressingQuantity ? `<li>${data.dressingQuantity} dressinger</li>` : ""}
      </ul>
...
async function sendBestillingEmail(subject, htmlContent, toAddresses, ccAddresses = []) {
  const axios = require("axios");
  const accessToken = await getAzureAccessToken();

  const toRecipients = toAddresses.map((a) => ({ emailAddress: { address: a } }));
  const ccRecipients = ccAddresses
    .filter((e) => !toAddresses.includes(e))
...
exports.sendCateringNotification = onCall(
  ...
  async ({ data }) => {
    ...
    try {
      await sendBestillingEmail(
        `Ny cateringforespørsel: ${data.name}`,
        buildCateringEmailHtml(data),
        BESTILLING_INTERNAL_RECIPIENTS,
      );
    } catch (err) {
      logger.error("Catering internal email failed", { error: err?.message });
    }

    try {
      await sendBestillingEmail(
        "Vi har mottatt din cateringforespørsel — Crust n' Trust",
        buildCateringConfirmationHtml(data),
        [data.email],
      );
    } catch (err) {
      logger.error("Catering confirmation email failed", { error: err?.message });
    }
...
exports.sendLargeOrderNotification = onCall(
  ...
  async ({ data }) => {
    ...
    try {
      await sendBestillingEmail(
        `Ny storbestilling: ${data.name} (${data.pizzaQuantity || 0} pizzaer)`,
        buildLargeOrderEmailHtml(data),
        BESTILLING_INTERNAL_RECIPIENTS,
      );
    } catch (err) {
      logger.error("Large order internal email failed", { error: err?.message });
    }

    try {
      await sendBestillingEmail(
        "Vi har mottatt din storbestilling — Crust n' Trust",
        buildLargeOrderConfirmationHtml(data),
        [data.email],
      );
    } catch (err) {
      logger.error("Large order confirmation email failed", { error: err?.message });
    }
