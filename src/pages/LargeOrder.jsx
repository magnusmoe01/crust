...
function LargeOrder() {
  ...
  const estimatedTotal = useMemo(() => {
    ...
    return (
      pq * prices.pizzaPrice +
      sq * prices.sodaPrice +
      dq * prices.dressingPrice
    );
  ...
  function validate() {
    ...
    if (!form.deliveryDate) errs.deliveryDate = "Velg dato og tid";
    return errs;
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      const orderData = {
        ...
        priceSnapshot: {
          pizzaPrice: prices.pizzaPrice,
          sodaPrice: prices.sodaPrice,
          dressingPrice: prices.dressingPrice,
          estimatedTotal,
        },
        status: "pending",
      ...
      setSubmitError(
        "Noe gikk galt. Prøv igjen eller send e-post til event@crust.no.",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div className="large-order-page">
      {/* ── Hero ───────────────────────────────────── */}
      <section className="lo-hero">
        ...
        <a
          ...
          className="lo-hero-link"
        >
          For en mindre bestilling, klikk her{" "}
      ...
      {/* ── Order form ─────────────────────────────── */}
      <section
        className="lo-form-section"
        id="order-form"
        aria-label="Bestillingsskjema for storbestilling"
      >
        <h2>Storbestilling</h2>
        <p>Fyll ut skjemaet under for å sende en forespørsel.</p>
        ...
        <form className="lo-form" onSubmit={handleSubmit} noValidate>
          ...
          <div className="lo-form-row">
            <label>
              ...
              />
              {errors.name && (
                <span className="lo-field-error" role="alert">
                  {errors.name}
                </span>
              )}
            </label>
          ...
          <div className="lo-form-row-3">
            ...
            <label>
              Antall dressinger
              <select
                name="dressingQuantity"
                value={form.dressingQuantity}
                onChange={handleChange}
              >
                {DRESSING_QTYS.map((q) => (
                  <option key={q} value={q}>
            ...
            {pricesLoaded ? (
              <>
                <div>
                  ...
                  <div className="lo-price-total">
                    {estimatedTotal.toLocaleString("nb-NO")} kr
                  </div>
                </div>
                <p className="lo-price-note">
                  Endelig pris bekreftes av oss etter forespørselen er mottatt.
                </p>
              </>
            ) : (
          ...
          <label>
            Leveringskommentarer
            <textarea
              name="deliveryComments"
              value={form.deliveryComments}
              onChange={handleChange}
              placeholder="F.eks. inngang, heis, parkering..."
            />
          </label>

          <label>
            Ønsker for utvalg / smak
            <textarea
              name="selectionComments"
              value={form.selectionComments}
              onChange={handleChange}
              placeholder="F.eks. ekstra mange Margherita, ingen kjøtt, allergi..."
            />
          </label>

          {submitError && (
            <p className="lo-form-error" role="alert">
              {submitError}
            </p>
          )}
      ...
      {showSuccess && (
        <div
          className="lo-modal-backdrop"
          role="dialog"
          aria-modal="true"
          aria-label="Forespørsel sendt"
          onClick={(e) => {
        ...
        >
          <div className="lo-modal-card">
            ...
            <p>
              Vi kontakter deg for å bekrefte detaljer og se hvordan vi kan
              oppfylle bestillingen din.
            </p>
            <button
              type="button"
              className="lo-modal-close"
