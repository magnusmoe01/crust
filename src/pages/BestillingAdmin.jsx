...
function BestillingAdmin() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(PRICES_DOC, (snap) => {
      if (snap.exists()) {
        ...
        setPrevPrices({
          pizzaPrice: d.pizzaPrice,
          sodaPrice: d.sodaPrice,
          dressingPrice: d.dressingPrice,
        });
      }
      setLoading(false);
    });
  ...
  useEffect(() => {
    getDocs(query(AUDIT_COL, orderBy("changedAt", "desc"), limit(20))).then(
      (snap) => {
        setAuditLog(
          snap.docs.map((d) => ({
            id: d.id,
  ...
  useEffect(() => {
    ...
    const unsub1 = onSnapshot(q1, (snap) => {
      setCateringOrders(snap.docs.map((d) => ({ id: d.id, ...d.data() })));
    });

    const q2 = query(
      collection(db, "largeOrders"),
      orderBy("createdAt", "desc"),
      limit(15),
    );
    const unsub2 = onSnapshot(q2, (snap) => {
      setLargeOrders(snap.docs.map((d) => ({ id: d.id, ...d.data() })));
    });
  ...
  async function handleSave() {
    ...
    try {
      ...
      const changes = [];
      if (prevPrices && prevPrices.pizzaPrice !== pp) {
        changes.push({
          field: "pizzaPrice",
          oldValue: prevPrices.pizzaPrice,
          newValue: pp,
        });
      }
      if (prevPrices && prevPrices.sodaPrice !== sp) {
        changes.push({
          field: "sodaPrice",
          oldValue: prevPrices.sodaPrice,
          newValue: sp,
        });
      }
      if (prevPrices && prevPrices.dressingPrice !== dp) {
        changes.push({
          field: "dressingPrice",
          oldValue: prevPrices.dressingPrice,
          newValue: dp,
        });
      }
  ...
  if (loading) {
    return <div className="ba-loading">Laster priser...</div>;
  }

  return (
    <div className="bestilling-admin">
      <h1>Bestillingspriser</h1>
      <p>
        Endre prisene som brukes i priskalkulatoren for storbestillinger. Endringer
      ...
      <div className="ba-card">
          ...
          <div className="ba-field">
            <label>
              ...
              <input
                ...
                step="1"
                value={dressingPrice}
                onChange={(e) => setDressingPrice(e.target.value)}
              />
            </label>
            <span className="ba-unit">kr</span>
          </div>
        ...
        <div className="ba-actions">
          ...
          {status.msg && (
            <span className={`ba-status ${status.type}`}>{status.msg}</span>
          )}
        </div>
              ...
              {auditLog.map((entry) => {
                ...
                return (
                  <div key={entry.id} className="ba-audit-item">
                    <strong>{entry.field}</strong>: {entry.oldValue} →{" "}
                    {entry.newValue} kr — {entry.changedBy} — {dateStr}
                  </div>
                );
              })}
            </div>
      ...
      <div className="ba-orders-card">
        <h2>Siste cateringforespørsler</h2>
        {cateringOrders.length === 0 ? (
          <p className="ba-orders-empty">Ingen forespørsler ennå.</p>
        ) : (
          cateringOrders.map((o) => {
            ...
            return (
              <div key={o.id} className="ba-order-item">
                ...
                <div className="ba-order-meta">
                  {o.email} · {o.phone} · {o.date}
                  {d && ` · ${d.toLocaleDateString("nb-NO")}`}
                </div>
                {o.address && (
                  <div className="ba-order-meta">{o.address}</div>
                )}
              </div>
            );
          })
      ...
      <div className="ba-orders-card">
        ...
        {largeOrders.length === 0 ? (
          <p className="ba-orders-empty">Ingen bestillinger ennå.</p>
        ) : (
          largeOrders.map((o) => {
            const d = o.createdAt?.toDate?.();
            return (
              <div key={o.id} className="ba-order-item">
                <strong>{o.name}</strong> — {o.pizzaQuantity} pizza,{" "}
                {o.sodaQuantity} brus, {o.dressingQuantity} dressing
                <div className="ba-order-meta">
                  {o.email} · {o.phone}
                  {o.priceSnapshot?.estimatedTotal &&
                    ` · ${o.priceSnapshot.estimatedTotal.toLocaleString("nb-NO")} kr`}
                  {d && ` · ${d.toLocaleDateString("nb-NO")}`}
                </div>
                {o.address && (
                  <div className="ba-order-meta">{o.address}</div>
